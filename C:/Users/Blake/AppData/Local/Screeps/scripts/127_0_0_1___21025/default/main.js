...
function requireRoleCensusLimitUtility () {
	...
	class RoleCensusLimitUtility {
	    ...
	    static getAllLimits(rcl, roomState, roomName) {
	        ...
	        limits.scout = (commonjsGlobal.State && commonjsGlobal.State.scoutQueue && commonjsGlobal.State.scoutQueue.length > 0) ? 1 : 0;
...
function requireTaskAssignmentManager () {
	...
	class TaskAssignmentManager {
	    ...
	    static assignScout(creep, homeState) {
	        if (!commonjsGlobal.State.scoutQueue || commonjsGlobal.State.scoutQueue.length === 0) {
	        ...
	        creep.heap.targetRoom = commonjsGlobal.State.scoutQueue[0];
...
var Filler_1;
var hasRequiredFiller;

function requireFiller () {
	if (hasRequiredFiller) return Filler_1;
	hasRequiredFiller = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
...
var Scout_1;
var hasRequiredScout;

function requireScout () {
	if (hasRequiredScout) return Scout_1;
	hasRequiredScout = 1;
	const ActionConstants = requireActionConstants();
...
var Repairman_1;
var hasRequiredRepairman;

function requireRepairman () {
	if (hasRequiredRepairman) return Repairman_1;
	hasRequiredRepairman = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
	...
	Repairman_1 = Repairman;
	return Repairman_1;
}

var Defender_1;
var hasRequiredDefender;

function requireDefender () {
	if (hasRequiredDefender) return Defender_1;
	hasRequiredDefender = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
...
var MeleeCreep_1;
var hasRequiredMeleeCreep;

function requireMeleeCreep () {
	if (hasRequiredMeleeCreep) return MeleeCreep_1;
	hasRequiredMeleeCreep = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
	...
	MeleeCreep_1 = MeleeCreep;
	return MeleeCreep_1;
}

var RangerCreep_1;
var hasRequiredRangerCreep;

function requireRangerCreep () {
	if (hasRequiredRangerCreep) return RangerCreep_1;
	hasRequiredRangerCreep = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
	...
	const RangerCreep = {
	    run: function (creep) {
	        ...
	        if (actionIntent === ActionConstants.ACTION_FLEE) {
	            ...
	            if (fp) {
	                creep.moveTo(new RoomPosition(fp.x, fp.y, fp.roomName), {
	                    reusePath: 3,
	                    visualizePathStyle: { stroke: '#00aaff', opacity: 0.6 }
	                });
	            }
	            return;
	        }
	        ...
	        if (actionIntent === ActionConstants.ACTION_RANGED_ATTACK) {
	            ...
	            if (result === ERR_NOT_IN_RANGE) {
	                ...
	                creep.moveTo(target, {
	                    reusePath: 3,
	                    visualizePathStyle: { stroke: '#ff6600', opacity: 0.6 }
	                });
	            } else if (result === OK || result === ERR_INVALID_TARGET) {
	                creep.heap.state = 'idle';
	                creep.heap.actionIntent = ActionConstants.ACTION_IDLE;
	...
	RangerCreep_1 = RangerCreep;
	return RangerCreep_1;
}

var MedicCreep_1;
var hasRequiredMedicCreep;

function requireMedicCreep () {
	if (hasRequiredMedicCreep) return MedicCreep_1;
	hasRequiredMedicCreep = 1;
	const ActionConstants = requireActionConstants();
	const GameObjectUtility = requireGameObjectUtility();
...
function requireRoleExecutor () {
	...
	roles['filler'] = requireFiller();
	roles['scout'] = requireScout();
	roles['repairman'] = requireRepairman();
	roles['defender'] = requireDefender();
	roles['meleeCreep'] = requireMeleeCreep();
	roles['rangerCreep'] = requireRangerCreep();
	roles['medicCreep'] = requireMedicCreep();
...
function requireTowerManager () {
	...
	class TowerManager {
	    static run() {
	        ...
	        commonjsGlobal.State.rooms.forEach((roomState, roomName) => {
...
function requireMilitaryManager () {
	...
	class MilitaryManager {
	    ...
	    static buildAggressiveZoneQueue() {
	        ...
	        commonjsGlobal.State.militaryQueue = scores.slice(0, 3).map(s => s.roomName);
	    }
