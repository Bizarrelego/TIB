...
function runSKOperations() {
    ...
    for (const roomName of global.State.scannedRooms) {
        ...
        // `lairs` from global.State could be an Array or Map based on discoveryManager structure
        // structuresByRoom is Maps of Maps
        ...
        // Only dispatch every 50 ticks to save CPU
        ...
        if (skMinersCount < requiredMiners) {
            ...
            for (const source of sources) {
                if (!assignedSources.has(source.id)) {
                    ...
                    const cost = 1000; // 700 + 50 + 200 = 950
        ...
        // Dispatch skGuard
        if (skGuardsCount < 1) { // 1 guard per room usually handles all lairs if built well
            ...
            const guardCost = 500 + 560 + 750; // 1810
